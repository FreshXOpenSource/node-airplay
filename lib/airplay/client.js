...
Client.prototype.disconnectSS = function()
{
  if (this.ssocket_) {
    this.ssocket_.destroy();
  }
  this.ssocket_ = null;
}

Client.prototype.connectSS = function(nextFile, callback)
{
  var self = this
  ...
  self.ssocket_.on('data', function(data) {
    ...
    if(res != null)
    {
        ...
        // console.log("SS", res)
        var m = data.toString().match(/\/slideshows\/\d+\/assets\/(\d+)/)
        
        nextFile(m[1], function(file){
            if(!self.ssocket_) return
            self.ssocket_.write(
                'HTTP/1.1 200 OK\n'+
...
Client.prototype.connectPTTH = function(eventHandler, callback)
{
  var self = this
...
Client.prototype.parseRequest_ = function(res) {
  ...
  if(header.match(/event/) || header.match(/slideshow/))
  {
    return body 
  }
  else{
