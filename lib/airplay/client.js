...
var Client = function(host, port, user, pass, callback) {
  ...
  this.rsocket_ = net.createConnection(port, host);
  this.rsocket_.on('connect', function() {

    self.rsocket._write(
      'POST /reverse\n' +
      'Upgrade: PTTH/1.0\n' + 
      'Connection: Upgrad\n' + 
      'X-Apple-Purpose: even\n' +
      'Content-Length: 0\n' +
  ...
  this.rsocket_.on('data', function(data) {
...
Client.prototype.issue_ = function(req, body, callback) {
  ...
  req.headers['X-Apple-AssetKey'] = '1bd6ceeb-fffd-456c-a09c-996053a7a08c';
  if(req.transition){
  	req.headers['X-Apple-Transition'] = req.transition;
  }
...
Client.prototype.put = function(path, transition, body, callback) {
  var req = {
    method: 'PUT',
    path: path,
